...
class TokenType(BaseModel):
    ...
    base_units_per_main: int = Field(
        description="Base units per main token (e.g., 1,000,000 µSTX per STX)"
    )
    decimal_places: int = Field(
        description="Default decimal places for main unit display"
    )
...
class TokenAmount(BaseModel):
    ...
    base_units_value: int = Field(
        description="Amount in base units (e.g., microSTX, satoshi)"
    )
    ...
    def __init__(
        ...
        token_type: TokenType = STACKS_TOKEN,
        **kwargs,
    ):
        """
    ...
    def __add__(self, other: Union["TokenAmount", int, float]) -> "TokenAmount":
        ...
        if isinstance(other, TokenAmount):
            ...
            return TokenAmount(
                ...
                + int(other * self.token_type.base_units_per_main),
                token_type=self.token_type,
            )
    ...
    def __mul__(self, multiplier: Union[int, float]) -> "TokenAmount":
        ...
        if isinstance(multiplier, (int, float)):
            ...
            return TokenAmount(
                base_units=int(self.base_units_value * multiplier),
                token_type=self.token_type,
            )
    ...
    def __truediv__(self, divisor: Union[int, float]) -> "TokenAmount":
        ...
        if isinstance(divisor, (int, float)) and divisor != 0:
            # Truncate to integer base units to maintain precision
            return TokenAmount(
                base_units=int(self.base_units_value / divisor),
                token_type=self.token_type,
            )
