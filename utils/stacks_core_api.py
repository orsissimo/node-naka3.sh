...
class StacksCoreAPI:
    ...
    def __init__(
        self, base_url: str = "http://localhost:20443", auth_token: Optional[str] = None
    ):
...
class StacksCoreAPIWrapper:
    ...
    def get_account_info_result(self, address: str) -> ApiResult:
        ...
        return self._safe_execute(
            "get_account_info", lambda: self.api.get_account_info(address)
        )
    ...
    def wait_for_tx_confirmation(self, txid: str, timeout: int = 60) -> bool:
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                logger.error(
                    f"Unexpected error during transaction confirmation wait: {e}"
                )
                time.sleep(3)

        raise StacksTimeoutException(
            f"Transaction confirmation timeout after {timeout}s for txid: {txid}"
        )
    ...
    def get_balance_in_stx(self, address: str) -> float:
        ...
        result = self._safe_execute(
            "get_balance_in_stx", lambda: self.api.get_account_info(address)
        )
    ...
    def get_info_safe(self) -> ApiResult:
        ...
        return self._safe_execute("get_info", lambda: self.api.get_info())
