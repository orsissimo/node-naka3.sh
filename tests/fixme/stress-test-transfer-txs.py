...
def submit_transfer_batch(miner: Miner, transfers: list) -> list:
    ...
    for i, transfer in enumerate(transfers):
        try:
            ...
            logger.standard(
                f"{i+1:3d}/{len(transfers)}",
                f"TX: {txid[:8]}... - Amount: {transfer.amount} - Nonce: {nonce} - Memo: {transfer.memo}",
            )
...
def wait_and_verify_transfers(submitted_transfers: list, timeout: int = 300) -> dict:
    ...
    while time.time() - start_time < timeout:
        ...
        if current_height > last_verified_height:
            print(
                f"New block mined! Height: {current_height} (+{current_height - initial_height})"
            )
...
def verify_transfers(submitted_transfers: list) -> dict:
    ...
    for transfer_info in submitted_transfers:
        ...
        try:
            ...
            logger.success(
                f"✓ Transfer {transfer_info.amount} µSTX - TXID: {transfer_info.txid}"
            )
...
def main():
    ...
    try:
        ...
        successful_submissions = [
            t for t in submitted_transfers if t.status == TransactionStatus.SUBMITTED
        ]
        failed_submissions = [
            t for t in submitted_transfers if t.status == TransactionStatus.FAILED
        ]
        ...
        if verification_results["confirmed"] > 0:
            ...
            confirmed_transfers = verification_results["details"].confirmed[
                :3
            ]  # Show first 3
            for transfer in confirmed_transfers:
                logger.standard("TXID", transfer.txid)
                logger.standard(
                    "Amount", f"{transfer.amount} µSTX, Memo: {transfer.memo}"
                )
